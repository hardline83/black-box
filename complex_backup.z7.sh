...
LOG_BASE_DIR="/var/log/backups"
LOG_DIR="${LOG_BASE_DIR}/db/${DB_HOST}"
ZABBIX_LOG_DIR="${LOG_BASE_DIR}/zabbix"
LOG_FILE="${LOG_DIR}/${BACKUP_NAME}.log"
ZABBIX_LOG_FILE="${ZABBIX_LOG_DIR}/${BACKUP_NAME}.zabbix.log"

# Создаем структуру каталогов для логов
mkdir -p "$LOG_DIR" "$ZABBIX_LOG_DIR" "$TMP_DIR"
...
# Функция для записи метрик в лог Zabbix
log_zabbix() {
    local metric_name="$1"
    local metric_value="$2"
    echo "${HOSTNAME} ${metric_name} ${metric_value}" >> "$ZABBIX_LOG_FILE"
}

# Функция для записи статуса операции
...
# Функция для записи всех метрик в лог Zabbix
finalize_zabbix_log() {
    local total_time=$(calculate_duration "$total_time_start" "$(get_timestamp)")
    
    # Записываем все метрики
    log_zabbix "backup.dump.status" "$DUMP_STATUS"
    log_zabbix "backup.compress.status" "$COMPRESS_STATUS"
    log_zabbix "backup.encrypt.status" "$ENCRYPT_STATUS"
    log_zabbix "backup.upload.status" "$UPLOAD_STATUS"
    log_zabbix "backup.script.status" "$SCRIPT_STATUS"
    log_zabbix "backup.dump.time" "$DUMP_TIME"
    log_zabbix "backup.compress.time" "$COMPRESS_TIME"
    log_zabbix "backup.encrypt.time" "$ENCRYPT_TIME"
    log_zabbix "backup.upload.time" "$UPLOAD_TIME"
    log_zabbix "backup.total.time" "$total_time"
    log_zabbix "backup.dump.size" "$DUMP_SIZE"
    log_zabbix "backup.compressed.size" "$COMPRESSED_SIZE"
}
...
prepare_directories() {
    ...
    mkdir -p "$DUMP_DIR" "$ARCHIVE_DIR" "$LOG_DIR" "$TMP_DIR"
...
create_db_dump() {
    if $SKIP_DUMP; then
        ...
        if [ ! -f "$SOURCE_DUMP" ]; then
            ...
            log_operation_status "dump" "failed" 0 0
...
*Проблема:* Файл дампа не найден: $SOURCE_DUMP"
        ...
        log_operation_status "dump" "skipped" 0 "$(get_size "$SOURCE_DUMP")"
        return 0
    fi
    ...
    if pg_dump -U "$DB_USER" "$DATABASE" -h "$DB_HOST" -p "$DB_PORT" > "$SOURCE_DUMP" 2>>"${DUMP_DIR}/pg_dump_error_mes.log"; then
        ...
        log_operation_status "dump" "success" "$dump_dur" "$(get_size "$SOURCE_DUMP")"
...
upload_all_to_obs() {
    ...
    for file in "${files[@]}"; do
        ...
        if upload_to_obs "$file"; then
            ...
            log_operation_status "upload" "failed" 0
            SCRIPT_STATUS="failed"
...
*Статус:* Прерывание выполнения"
    ...
    log_operation_status "upload" "success" "$upload_dur"
...
*Доступно места:* \`$(numfmt --to=iec "$available_space")\`"
    ...
    log_operation_status "compress" "success" "$compress_dur" "$compressed_size"
    ...
    log_operation_status "encrypt" "success" "$encrypt_dur"
    ...
    log "📊 Лог для Zabbix: $ZABBIX_LOG_FILE"
