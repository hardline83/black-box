...
# Zabbix Logging
ZABBIX_LOG_DIR="${SCRIPT_DIR}/zabbix_logs"
ZABBIX_LOG="${ZABBIX_LOG_DIR}/zabbix_$(date +%Y%m%d_%H%M%S).json"
...
log_zabbix() {
    ...
    local status="$2"
    local duration="${3:-0}"
    local size_before="${4:-0}"
    local size_after="${5:-0}"
    
    mkdir -p "$ZABBIX_LOG_DIR"
    
    # Создаем/обновляем JSON-лог
...
EOF
    ...
    jq --arg stage "$stage" \
       ...
       --argjson size_before "$size_before" \
       --argjson size_after "$size_after" \
       '.stages[$stage] = {
           status: $status,
           duration: $duration,
           size_before: $size_before,
           size_after: $size_after
       }' "$ZABBIX_LOG" > "${ZABBIX_LOG}.tmp" && mv "${ZABBIX_LOG}.tmp" "$ZABBIX_LOG"
}
...
*Статус:* Проверка подключения не удалась"
        log_zabbix "db_connection" "failed" "${check_dur%.*}"
        exit 1
    fi
...
check_deps() {
    local missing=()
    for cmd in tar pigz openssl obsutil split pg_dump psql jq; do
...
*Проблема:* Отсутствуют зависимости - ${missing[*]}"
        log_zabbix "dependencies" "failed" 0
        exit 1
    fi
...
*Проблема:* Отсутствует конфиг obsutil"
        log_zabbix "configuration" "failed" 0
        exit 1
    fi
}
...
Требуется: $(numfmt --to=iec ${needed}K), Доступно: $(numfmt --to=iec ${available}K)"
        log_zabbix "disk_space" "failed" 0
        exit 1
    fi
}
...
clean_old_backups() {
    ...
    log_zabbix "cleanup" "success" "${clean_dur%.*}"
}
...
*Проблема:* Файл дампа не найден: $SOURCE_DUMP"
            log_zabbix "db_dump" "failed" 0
            exit 1
        fi
        
        log "ℹ️ Используется существующий дамп: $SOURCE_DUMP"
        log_zabbix "db_dump" "skipped" 0 "$(get_size "$SOURCE_DUMP")"
        return 0
    fi
    ...
    if pg_dump -U "$DB_USER" "$DATABASE" -h "$DB_HOST" -p "$DB_PORT" > "$SOURCE_DUMP" 2>>"${DUMP_DIR}/pg_dump_error_mes.log"; then
        ...
        log_zabbix "db_dump" "success" "${dump_dur%.*}" "$dump_size"
    else
...
*Статус:* Ошибка при создании дампа"
        log_zabbix "db_dump" "failed" "${dump_dur%.*}"
        exit 1
    fi
...
Код ошибки: $exit_code"
        log_zabbix "split" "failed" "${split_dur%.*}"
        exit 1
    fi
...
upload_all_to_obs() {
    ...
    for file in "${files[@]}"; do
        if [ "$file" = "$LOG_FILE" ] || [ "$file" = "$ZABBIX_LOG" ]; then
            continue
        fi
...
*Статус:* Прерывание выполнения"
            log_zabbix "upload" "failed" "$(echo "$(date +%s.%N) - $upload_start" | bc)" "$total_files" "$uploaded_files"
            exit 1
        fi
    done

    log "\n📝 Загрузка лог-файлов в OBS"
    ...
    if upload_to_obs "$ZABBIX_LOG"; then
        ((uploaded_files++))
    else
        log "⚠️ Не удалось загрузить Zabbix-лог, но продолжаем выполнение"
    fi

    local upload_end=$(date +%s.%N)
    local upload_dur=$(echo "$upload_end - $upload_start" | bc)
    log "✅ Загружено $uploaded_files/$total_files файлов за $(format_duration ${upload_dur%.*})"
    log_zabbix "upload" "success" "${upload_dur%.*}" "$total_files" "$uploaded_files"
}
...
*Статус:* Подключение успешно"
            log_zabbix "dry_run" "success" "$(echo "$(date +%s.%N) - $total_time_start" | bc)"
            exit 0
        fi
    fi
    ...
    local source_size=$(get_size "$SOURCE")

    log "✅ Сжатие завершено за $(format_duration ${compress_dur%.*})"
    log "📊 Результат: $(numfmt --to=iec $compressed_size) (коэф. $(echo "scale=2; $source_size/$compressed_size" | bc)x)"
    log_zabbix "compression" "success" "${compress_dur%.*}" "$source_size" "$compressed_size"
    ...
    local encrypted_size=$(get_size "$ENCRYPTED_FILE")

    log "✅ Шифрование завершено за $(format_duration ${encrypt_dur%.*})"
    log "📦 Размер зашифрованного файла: $(numfmt --to=iec $encrypted_size)"
    log_zabbix "encryption" "success" "${encrypt_dur%.*}" "$compressed_size" "$encrypted_size"
    ...
    log "🗃️ Коэффициент сжатия: $(echo "scale=2; $source_size/$compressed_size" | bc)x)"
    log "📝 Лог-файл: $LOG_FILE"
    log "📊 Zabbix-лог: $ZABBIX_LOG"
...
*Исходный размер:* \`$(numfmt --to=iec $source_size)\`
*Сжатый размер:* \`$(numfmt --to=iec $compressed_size)\` (\`$(echo "scale=2; $source_size/$compressed_size" | bc)x\`)
...
*Лог-файл:* \`${LOG_FILE}\`"
    ...
    log_zabbix "complete" "success" "${total_dur%.*}"
}
...
*Статус:* ❌ Критическая ошибка"
    log_zabbix "complete" "failed" "$(echo "$(date +%s.%N) - $total_time_start" | bc)"
    exit 1
fi
