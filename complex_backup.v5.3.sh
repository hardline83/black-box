...
main() {
    # Инициализация
    prepare_directories
    prepare_temp_dir  # Перемещено сюда для подготовки перед проверкой места
...
*OBS путь:* \`${OBS_BASE_PATH}\`"
    ...
    if ! $SKIP_DUMP || $DRY_RUN; then
        ...
        check_s3_connection
        check_deps
        check_disk_space  # Теперь выполняется после prepare_temp_dir
...
*Размер дампа:* \`$(numfmt --to=iec "$(get_size "$SOURCE")")\`
*Размер архива:* \`$(numfmt --to=iec "$compressed_size")\`
*Коэффициент сжатия:* \`$(echo "scale=2; $(get_size "$SOURCE")/$compressed_size" | bc)x\`
*Путь в OBS:* \`${OBS_BASE_PATH}/${BACKUP_DATE}/\`"
...
trap 'log "❌ Произошла ошибка в строке $LINENO"; exit 1' ERR
