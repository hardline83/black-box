...
LOG_FILE="/backup-data/dev-blackbox/arch_and_enc_${BACKUP_NAME}.log"
...
upload_to_s3() {
    ...
    local exit_code=$?
    local end=$(date +%s.%N)
    local duration=$(printf "%.2f" $(echo "$end - $start" | bc))
...
COMPRESS_DUR=$(printf "%.2f" $(echo "$COMPRESS_END - $COMPRESS_START" | bc))
...
ENCRYPT_DUR=$(printf "%.2f" $(echo "$ENCRYPT_END - $ENCRYPT_START" | bc))
...
# Итоговая информация
echo -e "\n=== СВОДКА ==="
echo "Общее время: $(format_duration $(echo "$(date +%s.%N) - $(date -d "@${COMPRESS_START%.*}" +%s.%N)" | bc))"
echo "Исходный размер: $(numfmt --to=iec $ORIGINAL_SIZE)"
