...
# ==================== КОНФИГУРАЦИЯ ====================
OBS_BUCKET="coca-cola"                  # Имя бакета в OBS
OBS_CONFIG_FILE="$HOME/.obsutilconfig"  # Конфиг для obsutil (домашний каталог)
...
CHUNK_SIZE="50GB"                       # Размер частей для split
...
# Создаем лог-каталог если его не существует
...
check_disk_space() {
    local needed=$(($(get_size "$SOURCE") * 2 / 1024))  # KB с запасом (x3 для частей)
